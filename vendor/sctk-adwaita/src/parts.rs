        ...
        let surface = if let Some(inner) = inner {
            surface::setup_surface(
                ...
                Some(move |dpi, surface: WlSurface, ddata: DispatchData| {
                    ...
                    // Do NOT commit here: the old buffer is still sized for the
                    // previous scale and would violate the Wayland buffer_scale
                    // constraint (buffer dims must be multiples of buffer_scale).
                    // FrameRequest::Refresh will re-render and commit with the
            ...
            surface::setup_surface(
                ...
                Some(move |dpi, surface: WlSurface, _ddata: DispatchData| {
                    surface.set_buffer_scale(dpi);
                    // Do NOT commit here for the same reason as above; border
                    // subsurfaces will be redrawn by the next full Refresh.
                }),
            )
        };
