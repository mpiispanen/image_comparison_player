...
impl Bitmap {
    ...
    pub fn buffer(&self) -> &[u8] {
        let len = (self.pitch().abs() * self.rows()) as usize;
        if len == 0 {
            // When a glyph has no pixels (e.g. space or zero-height bitmap),
            // the buffer pointer may be null. Rust 1.81+ UB checks reject
            // slice::from_raw_parts(null, 0), so we short-circuit here.
            return &[];
        }
        unsafe {
